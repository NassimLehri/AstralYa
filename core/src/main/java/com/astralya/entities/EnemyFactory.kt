...
object EnemyFactory {
    ...
    fun createGolemCristal()     = Enemy("golem_cristal","Golem de Cristal",Element.STELLAR,280,38,45,8,15,
        listOf(EnemySkill("Poing de Roc",65),EnemySkill("Éclat Cristallin",50,hitAll=true)),80,45,listOf("fragment_cristal","pierre_dure"))
    ...
    fun createGardienTemple()    = Enemy("gardien_temple","Gardien du Temple",Element.LIGHT,350,55,48,22,40,
        listOf(EnemySkill("Lame Sacrée",80),EnemySkill("Jugement",65,hitAll=true),EnemySkill("Bouclier Divin",0,statusEffect=StatusEffect.SHIELDED)),120,70,listOf("rune_temple"))
    fun createAutomate()         = Enemy("automate_volant","Automate Volant",Element.NEUTRAL,420,62,55,40,28,
        listOf(EnemySkill("Laser Précis",90),EnemySkill("Explosion Mécanique",70,hitAll=true)),150,90,listOf("engrenage_ancien","huile_mecanique"))
    ...
    fun createMorvax()           = Enemy("morvax","Morvax — Seigneur du Néant",Element.DARK,9999,120,80,65,110,
        listOf(EnemySkill("Abîme du Néant",180,hitAll=true),EnemySkill("Corruption Stellaire",150,statusEffect=StatusEffect.POISON),
               ...
               EnemySkill("Néant Absolu",350,hitAll=true)),9999,5000,listOf("coeur_neant","cristal_noir"),isBoss=true)
    ...
    private fun factoriesForZone(mapId: String): List<() -> Enemy> = when (mapId) {
        "foret_enchantee" -> listOf(::createSlimeVert, ::createLoupSombre, ::createFeeCorrompue)
        "grotte_cristal"  -> listOf(::createGolemCristal, ::createChauveSouris)
        "desert_oublie"   -> listOf(::createSerpentSable, ::createScarabeeAncien)
        "temple_etoiles"  -> listOf(::createGardienTemple)
        "cite_volante"    -> listOf(::createAutomate)
